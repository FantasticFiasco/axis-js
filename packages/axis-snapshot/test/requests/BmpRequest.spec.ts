...
describe('get parameters request', () => {
    ...
    describe('#url', () => {
        test('should return URL without options', () => {
            // Act
            const request = new BmpRequest(connection);

            // Assert
            expect(request.url).toBe(`${connection.url}/axis-cgi/bitmap/image.bmp`);
        });
        ...
        test('should return URL with empty options', () => {
            ...
            // Act
            const request = new BmpRequest(connection, options);
        ...
        test('should return URL with falsy JavaScript values', () => {
            ...
            // Assert
            expect(request.url).toBe(`${connection.url}/axis-cgi/bitmap/image.bmp?compression=0&rotation=0&squarepixel=0`);
        });
        ...
        test('should return URL with single option', () => {
            ...
            for (const { options, expectedQueryString } of testCases) {
                ...
                // Assert
                expect(request.url).toBe(`${connection.url}/axis-cgi/bitmap/image.bmp?${expectedQueryString}`);
            }
        });
